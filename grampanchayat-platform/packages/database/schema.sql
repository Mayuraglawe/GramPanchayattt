-- ============================================================
-- GRAM PANCHAYAT DIGITAL PLATFORM
-- Full PostgreSQL Schema — Auto-generated from Prisma
-- Version  : 1.0.0
-- Platform : Multi-tenant | Shared DB, Separate Schema per GP
-- Database : PostgreSQL 15+
-- Generated: 2026-08-11
-- ============================================================
--
-- MODULES COVERED:
--   01. GP Registry & Config       (gp_registry, gp_config)
--   02. Users & Auth               (users, otp_sessions, push_subscriptions, consent_logs)
--   03. Elected Members            (members)
--   04. Certificate Applications   (certificate_applications)
--   05. Property Tax               (properties, tax_payments)
--   06. Water Bills                (water_connections, water_bills)
--   07. Complaints / Grievances    (complaints)
--   08. Projects / Works           (projects, project_progress_updates)
--   09. Budget                     (budget_heads, budget_entries)
--   10. Gram Sabha Meetings        (gram_sabha)
--   11. Notices & Announcements    (notices)
--   12. Government Schemes         (schemes, scheme_eligibilities)
--   13. RTI Requests               (rti_requests)
--   14. AI Agent Permission System (agent_permission_requests,
--                                   agent_permission_approvals,
--                                   agent_audit_logs,
--                                   agent_dead_letter_queue)
--   15. Notifications              (notifications)
--   16. Audit Log                  (audit_logs)
--   17. Reports                    (reports)
--
-- SECURITY NOTES:
--   - aadhaar_last4 is CHAR(4) only — full Aadhaar NEVER stored
--   - mobile is encrypted AES-256 at app layer before insert
--   - Razorpay tokens only — no raw card/bank data columns exist
--   - All certificate PDFs in private S3; signed URLs expire in 15min
--   - Row-Level Security (RLS) must be enabled per GP schema
-- ============================================================
...
-- CreateEnum
CREATE TYPE "UserRole" AS ENUM ('CITIZEN', 'OPERATOR', 'GRAMSEVAK', 'SARPANCH', 'ADMIN');
